...
if(!isset($_SESSION['examineeSession']['examineenakalogin']) || $_SESSION['examineeSession']['examineenakalogin'] !== true) {
...
<div class="app-main__inner">
    ...
    <div class="col-md-12 p-0 mb-4">
        ...
        <table class="align-middle mb-0 table table-borderless table-striped table-hover" id="tableList">
            <?php
    $limitQuery = "SELECT $exDisplayLimit FROM exam_question_tbl WHERE exam_id='$examId'";
    $limitResult = $conn->query($limitQuery);
    $limitRow = $limitResult->fetch(PDO::FETCH_ASSOC);
    $limitValue = $limitRow[$exDisplayLimit];
    $limit1 = $limitValue / 5;
?>
        ...
        $selQuest = $conn->query("
    (SELECT * FROM exam_question_tbl 
    WHERE exam_id='$examId' AND exam_unit='I'
    ORDER BY RAND() 
    LIMIT $limit1)
    UNION 
    (SELECT * FROM exam_question_tbl 
    WHERE exam_id='$examId' AND exam_unit='II'
    ORDER BY RAND() 
    LIMIT $limit1)
    UNION 
    (SELECT * FROM exam_question_tbl 
    WHERE exam_id='$examId' AND exam_unit='III'
    ORDER BY RAND() 
    LIMIT $limit1)
    UNION 
    (SELECT * FROM exam_question_tbl 
    WHERE exam_id='$examId' AND exam_unit='IV'
    ORDER BY RAND() 
    LIMIT $limit1)
    UNION 
    (SELECT * FROM exam_question_tbl 
    WHERE exam_id='$examId' AND exam_unit='V'
    ORDER BY RAND() 
    LIMIT $limit1)
");
